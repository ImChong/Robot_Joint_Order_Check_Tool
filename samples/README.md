...
| 文件 / File | 来源 / Upstream |
| --- | --- |
| `g1_29dof_rev_1_0.urdf` | [unitree_ros / robots/g1_description](https://github.com/unitreerobotics/unitree_ros/blob/master/robots/g1_description/g1_29dof_rev_1_0.urdf) |
| `g1_29dof_rev_1_0.xml` | [unitree_ros / robots/g1_description](https://github.com/unitreerobotics/unitree_ros/blob/master/robots/g1_description/g1_29dof_rev_1_0.xml) |
...
这些文件均**原样收录、未作修改**，只是这里不带 `meshes/`（本工具只读 link / joint 拓扑，不需要网格）。
版权归 Unitree Robotics 所有，按 BSD 3-Clause 许可分发，许可证全文见 [`LICENSE.unitree_ros`](LICENSE.unitree_ros)。

`g1_29dof_rev_1_0` 是机身 29 自由度（无灵巧手），也是 [ParkourFormer](https://arxiv.org/abs/2605.25782) 等跑酷策略实际用的那一套；`g1_29dof_with_hand_rev_1_0` 在此基础上加了双手。

All four files are **verbatim, unmodified copies** from [unitreerobotics/unitree_ros](https://github.com/unitreerobotics/unitree_ros);
...
`g1_29dof_rev_1_0` is the 29-DOF body (no hands) that parkour policies such as
[ParkourFormer](https://arxiv.org/abs/2605.25782) actually deploy;
`g1_29dof_with_hand_rev_1_0` adds the dexterous hands on top.

这些示例在页面上是点击时从 `samples/` 读取的，所以用 `file://` 直接打开页面时无法载入 —— 请用
`python3 -m http.server` 之类的本地服务器，或把文件拖进页面。

These are fetched from `samples/` when their button is clicked, so they do not load when the page
